...
public class MelFilter {
	...
	public MelFilter(double leftEdge, double centerFreq, double rightEdge, double initialFreq, double deltaFreq)
		 ...
		 * too close.
		 */
		if ((Math.round(rightEdge - leftEdge) == 0) || (Math.round(centerFreq - leftEdge) == 0)
				|| (Math.round(rightEdge - centerFreq) == 0)) {
			throw new IllegalArgumentException("Filter boundaries too close");
		}
		/**
		 * Let's compute the number of elements we need in the <code>weight</code> field by computing how many frequency
		 ...
		 * initialize and update two variables in the <code>for</code> line.
		 */
