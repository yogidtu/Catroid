...
public class CopyProjectTask extends AsyncTask<String, Long, Boolean> {
	...
	protected Boolean doInBackground(String... projectNameArray) {
		...
		try {
			File oldProjectRootDirectory = new File(UtilFile.buildProjectPath(oldProjectName));
			File newProjectRootDirectory = new File(UtilFile.buildProjectPath(newProjectName));
			...
			UtilFile.deleteDirectory(new File(UtilFile.buildProjectPath(newProjectName)));
